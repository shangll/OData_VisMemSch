...
for (n in subj_list){
  for (k in cond_list){
    trainData <- subset(df_exp1a,(subj==n)&(setsize!=16)&(cond==k))
    testData <- data.frame(setsize=c(16))
    lm_pred <- lm(formula=rt~setsize,trainData)
    log_pred <- lm(formula=rt~log(setsize,2),trainData)
    lm_16 <- predict(lm_pred,newdata=testData)
    log_16 <- predict(log_pred,newdata=testData)
    if(k=="within"){
      w_16_lm <- append(w_16_lm,lm_16[[1]])
      w_16_log <- append(w_16_log,log_16[[1]])
    }else{
      b_16_lm <- append(b_16_lm,lm_16[[1]])
      b_16_log <- append(b_16_log,log_16[[1]])
    }
  }
}
w_obs <- subset(df_exp1a,(setsize==16)&(cond=="within"))
b_obs <- subset(df_exp1a,(setsize==16)&(cond=="between"))
w_obs_rt <- w_obs$rt
b_obs_rt <- b_obs$rt

print('within: lm')
t_val <- t.test(w_16_lm,w_obs_rt)
t_val
dat <- data.frame(mss16=c(w_16_lm,w_obs_rt),
                  pred=rep(c("lm","rt"),each=length(subj_list)))
d <- dat %>% cohens_d(mss16~pred)
d
...
# ----------------------------
print('within: log2')
t_val <- t.test(w_16_log,w_obs_rt)
t_val
...
# ----------------------------
print('between: log2')
t_val <- t.test(b_16_log,b_obs_rt)
t_val
...
df_exp3$setsize <- as.numeric(as.character(df_exp3$setsize))
df_exp3_size$setsize <- as.numeric(as.character(df_exp3_size$setsize))
w_8_lm <- c()
b_8_lm <- c()
w_8_log <- c()
b_8_log <- c()
...
for (n in subj_list){
  for (k in cond_list){
    trainData <- subset(df_exp3,(subj==n)&(setsize!=8)&(cond==k))
...
w_obs <- subset(df_exp3,(setsize==8)&(cond=="within"))
b_obs <- subset(df_exp3,(setsize==8)&(cond=="between"))
w_obs_rt <- w_obs$rt
b_obs_rt <- b_obs$rt
