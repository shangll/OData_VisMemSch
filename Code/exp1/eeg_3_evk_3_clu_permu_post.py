...
from eeg_config import epoDataPath,resPath,grpFigPath,\
    subjList_final,event_dict_distr,cond_label_list,\
    postChans,recog_labels,n_permutations,save_fig
import mne
...
fig_pref = os.path.join(grpFigPath,'stats_clu_post_')
fname = '_epo.fif'
...
for subjN in subjList_final:
    ...
    epochs_list.append(epo.pick_channels(postChans))
...
for i_clu, clu_idx in enumerate(good_cluster_inds):
    ...
    #     evokeds,title=title,picks=ch_inds,axes=ax_signals,
    #     colors=colors,linestyles=linestyles,show=False,
    #     split_legend=False,truncate_yaxis='auto')
    # figName = '%s_gfp_'+'%d.png'%(i_clu+1)
    mne.viz.plot_compare_evokeds(
...
erp_recog['setsize'] = pd.to_numeric(erp_recog['setsize'])
...
behav_coeff = getCoeff(
    df_sch_mean,'setsize','rt',subjList_final)
