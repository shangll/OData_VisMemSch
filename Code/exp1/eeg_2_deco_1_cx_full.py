...
#%% --- --- --- --- --- --- libraries --- --- --- --- --- ---
from eeg_config import decoDataPath_full,resPath,\
...
# epoch
fileName = 'deco_full_data.csv'
figPath = set_filepath(decoFigPath,'epo')
tfr_tag = 'erp'

t_list = np.load(file=os.path.join(decoDataPath_full,'t_list.npy'),
                 allow_pickle=True)
t_points = len(t_list)
...
for pick_chs in ['simi']:
    if pick_chs=='eeg':
        ...
        decoFigPath_ch = set_filepath(figPath,'simiChans_full')
    ...
    odd_data = np.load(
        file=os.path.join(decoDataPath_full,'odd'+dataFileName),
        allow_pickle=True)
    odd_labels = np.load(
        file=os.path.join(decoDataPath_full,'odd'+labelFileName),
        allow_pickle=True)
    print('ODDBALL TASK LOADED ***')
    print('*')

    recogAll_data = np.load(
        file=os.path.join(decoDataPath_full,'recogAll'+dataFileName),
        allow_pickle=True)
    recogAll_labels = np.load(
        file=os.path.join(decoDataPath_full,'recogAll'+labelFileName),
        allow_pickle=True)
    ...
    for cond in recog_label_list:
        recog_data_dict[cond] = np.load(
            file=os.path.join(
                decoDataPath_full,'recog_%s'%cond+dataFileName),
            allow_pickle=True)
        recog_labels_dict[cond] = np.load(
            file=os.path.join(
                decoDataPath_full,'recog_%s'%cond+labelFileName),
            allow_pickle=True)
    ...
    for cond in sizeList:
        recogSize_data_dict[cond] = np.load(
            file=os.path.join(
                decoDataPath_full,'recog_%d'%cond+dataFileName),
            allow_pickle=True)
        recogSize_labels_dict[cond] = np.load(
            file=os.path.join(
                decoDataPath_full,'recog_%d'%cond+labelFileName),
            allow_pickle=True)
        print('CONDITION %s LOADED ***'%cond)
    print('*')
    ...
    if tag_savefile==1:
        if os.path.isfile(
                os.path.join(resPath,'deco_full_data_subj.csv')):
            acc_df_subj.to_csv(
                os.path.join(resPath,'deco_full_data_subj.csv'),
                mode='a',header=False,index=False)
        else:
