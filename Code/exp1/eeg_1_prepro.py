...
for subjN,fileName in zip(subjList,fileList_EEG):
    ...
    # raw.drop_channels(['AF3','AF4','AF7','AF8'])
    '''
    annot_muscle,scores_muscle = annotate_muscle_zscore(
        raw,ch_type='eeg',threshold=threshold_muscle,
        min_length_good=0.2,filter_freq=freq_muscle)
    # plot muscle noises
    fig,ax = plt.subplots()
    ax.plot(raw.times,scores_muscle)
    ax.axhline(y=threshold_muscle,color='r')
    ax.set(xlabel='Time (sec)',ylabel='Z-Score',
           title='Muscle Activity')
    figName = figName+'muscle_subj%d.png'%subjN
    title = 'Subject %d:'%subjN
    save_fig(fig,title,figName)

    raw.annotations.append(annot_muscle.onset,
                           annot_muscle.duration,
                           annot_muscle.description)
    title = 'muscle subject %d'%subjN
    fig = raw.copy().plot(
        n_channels=64,title=title,show=show_flg)
    figName = figName+'muscle_epo.png'%subjN
    title = 'Subject %d:'%subjN
    save_fig(fig,title,figName)
    '''
    ...
    figName = os.path.join(preFigPath,'7_ICA_')

    # ICA
    epo_ICA = get_ICA(epochs,subjN)
    del epochs

    # plot PSD & evoked check
    title = 'Subject %d: ICA'%subjN
    epo_check(epo_ICA,title,figName)

    # crop time: from -0.2 to 0.8
    epo_ICA = epo_ICA.crop(tmin=tmin,tmax=tmax,
                           include_tmax=True,
                           verbose=None)

    # trial by trial manually after ICA
    title = 'Subject %d: ICA'%subjN
    epo_ICA.plot(n_channels=chanN,
                 events=epo_ICA.events,
                 title=title,show=True)
    plt.show(block=show_flg)
    plt.close('all')
    fig = epo_ICA.plot(n_channels=chanN,
                       events=epo_ICA.events,
                       title=title,show=False)
    plt.show(block=False)
    fig.savefig(figName+'rej_bad_manu_ICA_subj%d.png'%subjN,
                bbox_inches='tight',
                dpi=300)
    plt.close('all')

    fig = epo_ICA.plot_drop_log(subject='%d'%subjN,
                                ignore=('IGNORED','ACC=0'),
                                show=show_flg)
    save_fig(fig,figName+'rej_log_bad_ICA_subj%d.png'%subjN)

    # plot PSD & evoked check
    title = 'Subject %d: Reject Bad Epochs (ICA)'%subjN
    epo_check(epo_ICA,title,figName+'rej_bad_ICA_')

    print('1.7 ICA FINISHED ***')

    # --- --- --- --- --- --- 1.8 Re-Reference to Average

    figName = os.path.join(preFigPath,'8_ref_')

    # Average
    epo_ICA = epo_ICA.set_eeg_reference(ref_channels='average')

    '''
    # REST
    epo_ICA.del_proj()
    sphere = mne.make_sphere_model(
        'auto','auto',epo_ICA.info)
    src = mne.setup_volume_source_space(
        sphere=sphere,exclude=30.,pos=15.)
    forward = mne.make_forward_solution(
        epo_ICA.info,trans=None,
        src=src,bem=sphere)

    epo_ICA = epo_ICA.set_eeg_reference('REST',forward=forward)
    ...
    print('1.8 RE-REFERENCE FINISHED ***')

    # --- --- --- --- --- ---
    # SAVE FILES (AFTER ICA)
    fname_epo_ICA = 'subj%d_ICA_ref_epo.fif'%subjN
    epo_ICA.save(os.path.join(
        epoDataPath,fname_epo_ICA),overwrite=True)
    print('FILE (AFTER ICA) SAVED ***')
    print('***')

    # --- --- --- --- --- --- 1.9 Baseline Correction

    figName = os.path.join(preFigPath,'9_detr_')

    epo_ICA.pick_types(eeg=True).apply_baseline(
        baseline=baseline)
    ...
    epo_ICA.save(fpath,overwrite=True)
