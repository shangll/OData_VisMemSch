...
# ----------------------------
# within
t_val <- t.test(w_resid_log,w_resid_lm,paired=TRUE,alternative="less")
# t_val
dat <- data.frame(resids=c(w_resid_log,w_resid_lm),
                  pred=rep(c("log","lm"),each=length(subj_list)))
d <- dat %>% cohens_d(resids~pred,paired=TRUE)
...
# ----------------------------
# between
t_val <- t.test(b_resid_log,b_resid_lm,paired=TRUE,alternative="less")
# t_val
dat <- data.frame(resids=c(b_resid_log,b_resid_lm),
                  pred=rep(c("log","lm"),each=length(subj_list)))
d <- dat %>% cohens_d(resids~pred,paired=TRUE)
...
# #
# subj_list <- 1:20
# w_obs_rt <- subset(df_exp1a,(setsize==16)&(cond=="within")&(subj%in%subj_list))$rt
# b_obs_rt <- subset(df_exp1a,(setsize==16)&(cond=="between")&(subj%in%subj_list))$rt
# w_16_lm <- subset(df_exp1a,(setsize==16)&(cond=="within")&(subj%in%subj_list))$lm
# w_16_log <- subset(df_exp1a,(setsize==16)&(cond=="within")&(subj%in%subj_list))$log
# b_16_lm <- subset(df_exp1a,(setsize==16)&(cond=="between")&(subj%in%subj_list))$lm
# b_16_log <- subset(df_exp1a,(setsize==16)&(cond=="between")&(subj%in%subj_list))$log
# 
# print('within: lm')
# t_val <- t.test(w_16_lm,w_obs_rt,paired=TRUE)
# # t_val
...
# # Objects
# subj_list <- 21:41
# subj_list <- subj_list[-which(subj_list==33)]
# w_obs_rt <- subset(df_exp1a,(setsize==16)&(cond=="within")&(subj%in%subj_list))$rt
# b_obs_rt <- subset(df_exp1a,(setsize==16)&(cond=="between")&(subj%in%subj_list))$rt
# w_16_lm <- subset(df_exp1a,(setsize==16)&(cond=="within")&(subj%in%subj_list))$lm
# w_16_log <- subset(df_exp1a,(setsize==16)&(cond=="within")&(subj%in%subj_list))$log
